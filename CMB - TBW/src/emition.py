...
def calculation(archive, noun, FactorEmissions, Typo):
	...
	for x in range(0, FEMatriz.shape[1]):
	 	categoryHabil = FEMatriz[0][x]
	 	categoryNHabil = 'NH' + FEMatriz[0][x]
	 	if categoryHabil != 'Pollutant':
	  		if categoryHabil != '':
	 			for i in range(2, FEMatriz.shape[0]):
	 	 			contaminante = FEMatriz[i][0]
	 	 			pollutant[contaminante][categoryHabil].append(round(float(FEMatriz[i][x]),8))
	 	 			pollutant[contaminante][categoryNHabil].append(round(float(FEMatriz[i][x]),8))
	  				pollutant[contaminante][categoryHabil].append(round(float(FEMatriz[i][x + 1]),8))
	  				pollutant[contaminante][categoryNHabil].append(round(float(FEMatriz[i][x + 1]),8))
	...
	for poll in pollutants:
			...
			if Typo == 'Habil':
				...
				for x in range(colIH, colFH): 
				  	...
				  	L = float(matriz[i][colL])/1000
				   	val = float(matriz[i][x]) * L 
				    #print float(matriz[i][x]), '*', L, '=', val
				   	val = round(val *  pollutant[poll][category][0], 8)

				 	uncertainty = (round((((float(matriz[i][x]) * L) * pollutant[poll][category][1])), 8))
				   	data[FID_LINK]['pollutants'][hour][Typo][category] = [val, uncertainty]
				...
				for x in range(colFH, colFNH): 
				  	category = matriz[0][x]
				  	#print category
					L = float(matriz[i][colL])/1000 #Div for 1000 convert
					val = float(matriz[i][x]) * L #Operation val emission 
					val = round(val *  pollutant[poll][category][0])
					#print float(matriz[i][x]), '*', L, '*', pollutant[poll][category][0], 'Result = ', round((((float(matriz[i][x]) * L) * pollutant[poll][category][0])), 8)
				 	uncertainty = (round((((float(matriz[i][x]) * L) * pollutant[poll][category][1])), 8)) #Operation uncertainty
					data[FID_LINK]['pollutants'][hour][Typo][category] = [val, uncertainty]
		...
		#print data
		for ID in data: 
			hours = data[ID]['pollutants']
			for hour in hours:
				categories = hours[hour][Typo] 
				sumaval = sumauncertainy = 0
				for category in categories:
					sumaval += categories[category][0]
					sumauncertainy += categories[category][1]
				
				data[ID]['pollutants'][hour][Typo] = [sumaval, sumauncertainy]
...
def finality(folder, noun):
	...
	#ID_Grilla = data.keys()
	for ID in data: 
		pollutants = data[ID]['pollutants']
		for pollutant in pollutants:
			hours = pollutants[pollutant]
			for hour in hours:
				suma = eval('+'.join(hours[hour]))
				data[ID]['pollutants'][pollutant][hour] = [suma]
	...
	for i in range(1, matriz.shape[0]):
		...
		for x in range(colHour+1, matriz.shape[1]):
			...
			if entrycategory.get(name) is None: 
				entrycategory[name] = [float(matriz[i][x])]
...
def categoryVechiclegrid(folder):
...
def categoryFUEL(data, noun, pollution, Typo): 
	data2 = {}
	keys = data.keys()
	for key in keys: 
